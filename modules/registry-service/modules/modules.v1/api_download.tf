...
resource "aws_api_gateway_integration" "download_GET" {
  ...
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${local.region_name}:dynamodb:action/GetItem"
  integration_http_method = "POST"
  credentials             = var.credentials_role_arn

  request_templates = {
    "application/json" = templatefile("${path.module}/files/download_request.tpl", { dynamo_table_name = var.dynamodb_table_name })
  }
}
...
resource "aws_api_gateway_integration_response" "download_GET_200" {
  ...
  response_templates = { "application/json" = file("${path.module}/files/download_response.template") }
}
